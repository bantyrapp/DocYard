...
**Trial balance to Yardi journal entry.** The Yardi tool you trust for property accounting. **eazybookz.com** — runs **in your browser**; no server or WiFi required for the main flow. Upload your Excel trial balance, set post month and journal date, and download a file ready to import into Yardi. You can also download Excel templates (Yardi JE, trial balance).

## License

Proprietary. Copyright (c) 2025 EazyBookz. All Rights Reserved. See [LICENSE](LICENSE).
...
The **Download** page (`/download`) lets users get the EazyBookz desktop app for their computer. It’s linked from the footer and the hamburger menu.

**Build installers per platform:**

1. Set the URL the app should load when packaged: in `electron/main.js` set `APP_URL` to your deployed web app (e.g. `https://eazybookz.com`), or use the `APP_URL` env var when building.

2. From the `electron/` folder, build for each OS (use the right machine or CI for each):
   - **Windows:** `npm run build:win` → installers in `electron/dist/` (e.g. `EazyBookz Setup 1.0.0.exe`).
...
3. Host the built files (e.g. GitHub Releases, S3, or your server) and point the web app at them with env vars so the Download page shows real links:
   ...
   `VITE_DOWNLOAD_WIN_URL=https://github.com/you/eazybookz/releases/download/v1.0.0/EazyBookz-Setup-1.0.0.exe`
...
2. **Client:** `cd client && npm run dev` → Vite starts (e.g. http://localhost:3000 or 3001). Open the URL and confirm the EazyBookz home page loads.
